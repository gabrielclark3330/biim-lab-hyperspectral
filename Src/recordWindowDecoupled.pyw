...
class WebCamThread(QThread):
    ...
    def run(self):
        ...
        sub_folder_name = f"recordingAt{time}"
        ...
        while self._run_flag:
            ...
            if grabbingImages:
                ...
                path = os.path.join(path, f"imageWriteDir/{sub_folder_name}")
...
#        self.baslerThread.setBinning=True
    ...
    def stopRecord(self):
        ...
        if(self.set_referance):
            ...
            filename = f"./.tempTime/timestamps_{time}.pick"
            os.makedirs(os.path.dirname(filename), exist_ok=True)
            f = open(filename, "wb")
            pickle.dump(self.baslerThread.ddTimeStamps,f)
            f.close()

            print(f"length of imageArr: {len(self.baslerThread.dd)}")
            print(f"length of timeStamps: {len(self.baslerThread.ddTimeStamps)}")
